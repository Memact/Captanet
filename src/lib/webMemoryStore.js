...
const LEGACY_WEB_DB_NAME = 'memact-web-memory'
const WEB_DB_NAME = 'captanet-web-memory'
...
const webDb = createMemoryDb(WEB_DB_NAME)
const legacyWebDb = LEGACY_WEB_DB_NAME === WEB_DB_NAME ? null : createMemoryDb(LEGACY_WEB_DB_NAME)
...
function isGenericBrowserApp(app) {
  const normalized = normalize(app).toLowerCase()
  return [
    'browser',
    'phone browser',
    'chrome',
...
async function readLegacyDbMemories() {
  if (!legacyWebDb) {
    return []
  }

  try {
    await legacyWebDb.open()
    const records = await legacyWebDb.memories.orderBy('occurred_at').reverse().limit(MAX_WEB_MEMORIES).toArray()
...
async function clearLegacyDbMemories() {
  if (!legacyWebDb) {
    return
  }

  try {
    await legacyWebDb.memories.clear()
  } catch {
    // Ignore cleanup failures for the legacy IndexedDB store.
  }
  ...
  try {
    await Dexie.delete(LEGACY_WEB_DB_NAME)
  } catch {
    // Ignore delete failures for the legacy IndexedDB store.
  }
}
...
async function ensureInitialized() {
  if (!initPromise) {
    initPromise = (async () => {
      ...
      const legacyDbMemories = Array.from(
        new Map((await readLegacyDbMemories()).map((memory) => [memory.fingerprint, memory])).values()
      )
      const migratedMemories = Array.from(
        new Map([...legacyDbMemories, ...legacyMemories].map((memory) => [memory.fingerprint, memory])).values()
      )

      if (migratedMemories.length) {
        await webDb.memories.bulkPut(migratedMemories)
        clearLegacyMemories()
      }
      await clearLegacyDbMemories()
    })()
  }
...
function createSuggestionSeed(memories) {
  ...
  memories.forEach((memory) => {
    ...
    if (memory.application && !isGenericBrowserApp(memory.application)) {
      addSuggestion(`app:${memory.application}`, {
        id: `app:${memory.application}`,
        category: 'Saved app',
        title: titleCase(memory.application),
        subtitle: `${titleCase(memory.application)} captured locally on this device.`,
        completion: titleCase(memory.application),
        timestamp: memory.occurred_at,
      })
    }

    if (memory.context_subject && memory.context_subject.toLowerCase() !== memory.domain.toLowerCase()) {
      addSuggestion(`subject:${memory.context_subject}`, {
        id: `subject:${memory.context_subject}`,
        category: 'Saved topic',
        title: compactText(memory.context_subject, 60),
        subtitle:
          compactText(memory.structured_summary || memory.content_text, 72) ||
          'Saved locally on this device.',
        completion: memory.context_subject,
        timestamp: memory.occurred_at,
      })
      ...
      addSuggestion(`title:${memory.title}`, {
        ...
        category: 'Saved page',
        title: compactText(memory.title, 46),
        subtitle:
          compactText(memory.structured_summary || memory.content_text, 72) ||
          'Saved locally on this device.',
        completion: memory.title,
        timestamp: memory.occurred_at,
      })
    }
    ...
    addSuggestion(`month:${memory.month_key}`, {
      ...
      title: formatMonthLabel(memory.month_key),
      subtitle: `Memories saved in ${formatMonthLabel(memory.month_key)}.`,
      completion: formatMonthLabel(memory.month_key),
      timestamp: memory.occurred_at,
    })
  })
...
function parseStructuredQuery(rawQuery) {
  ...
  const monthMatch = query.match(/what do i remember from\s+(.+)$/i)
  if (monthMatch?.[1]) {
    const maybeMonth = normalize(monthMatch[1])
    if (/^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\s+\d{4}$/i.test(maybeMonth)) {
      return { type: 'month', value: maybeMonth }
    }
  }

  const domainMatch = lowered.match(/(?:what do i remember from|what was i looking at on|what did i read on)\s+("?)([^"]+)\1$/i)
  if (domainMatch?.[2]) {
    return { type: 'domain', value: normalize(domainMatch[2]) }
  }

  const appMatch = query.match(/(?:how have i been using|what was i doing in)\s+(.+)$/i)
  if (appMatch?.[1]) {
    return { type: 'app', value: normalize(appMatch[1]) }
  }

  const topicMatch = query.match(/(?:what do i remember about|what did i save about|where did i see|find)\s+("?)([^"]+)\1/i)
  if (topicMatch?.[2]) {
...
function scoreMemory(memory, parsedQuery) {
  ...
  if (parsedQuery.type === 'app') {
    const app = normalize(memory.application).toLowerCase()
    if (!app) return 0
    if (!(app === loweredValue || app.includes(loweredValue))) return 0
    return 0.86 + recencyScore(memory.occurred_at) * 0.14
  }
